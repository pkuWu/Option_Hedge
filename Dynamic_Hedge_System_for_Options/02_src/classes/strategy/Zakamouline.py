...
class Zakamouline(StrategyBase):
    ...
    def get_hedging_position(self,greek_df,**kwargs):
        ...
        df_hedge.loc[:,'H0'] = kwargs['lambda']/(kwargs['gamma']*greek_df.loc[:,'stock_price']*np.power(greek_df.loc[:,'sigma'],2)*greek_df.loc[:,'left_time'])

        df_hedge.loc[:,'H1'] = 1.12*np.power(kwargs['lambda'],0.31)*\
                               np.power(greek_df.loc[:,'left_time'],0.05)*\
                               np.power((-kwargs['r']*greek_df.loc[:,'left_time']/greek_df.loc[:,'sigma']),0.25)*\
                               np.power(greek_df.loc[:,'gamma']/kwargs['gamma'],0.5)

        df_hedge.loc[:,'K'] = -4.76*np.power(kwargs['lambda'],0.78)/\
                              np.power(greek_df.loc[:,'left_time'],0.02)*\
                              np.power((-kwargs['r']*greek_df.loc[:,'left_time']/greek_df.loc[:,'sigma']),0.25)*\
        ...
        for i in range(0,df_hedge.shape[0]):
            if position_rate<df_hedge.loc[i,'low_bound'] or position_rate>df_hedge.iloc[i,'up_bound']:
                position = df_hedge.loc[i,'delta']*kwargs['size']//100*100
                position_rate = position/kwargs['size']
            df_hedge[i, 'position'] = position
            df_hedge[i,'position_rate'] = position_rate

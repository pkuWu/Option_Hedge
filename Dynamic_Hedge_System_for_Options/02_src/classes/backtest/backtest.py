...
class Backtest:
    ...
    def set_option_portfolio(self):
        ...
        self.option_fee = self.paras.get('option_fee')
    ...
    def run_backtest(self):
        ...
        self.df_backtest.loc[:,'delta_exposure_pnl'] = self.df_backtest.loc[:,'delta_pnl'] - self.df_backtest.loc[:,'stock_position'].shift(1).fillna(0)*self.df_backtest.loc[:,'stock_price'].diff().fillna(0)
        self.df_backtest.loc[:, 'stock_value'] = self.df_backtest.loc[:,'stock_price']*self.df_backtest.loc[:,'stock_position']
        self.df_backtest.loc[:, 'stock_pnl'] = self.df_backtest.loc[:, 'stock_price'].diff().fillna(0)*self.df_backtest.loc[:, 'stock_position'].shift(1).fillna(0)
        self.df_backtest.loc[:,'trading_cost'] = self.df_backtest.loc[:, 'stock_position'].diff().fillna(self.df_backtest['stock_position'][0]).abs() * \
        ...
        self.df_backtest.loc[:,'total_value'] = self.df_backtest.loc[:, 'cash_account'] - \
    ...
    def hedge_summary(self):
        self.hedge_pnl_summary = pd.Series(self.df_backtest.loc[:,'total_value'][-1], index=['total_value'])
        self.hedge_pnl_summary['option_value'] = self.option_fee-self.df_backtest.loc[:, 'option_value'][-1]
        self.hedge_pnl_summary['hedge_value'] = self.df_backtest.loc[:,'nav'][-1]
        ...
        self.hedge_pnl_summary['max_drawdown'] = self.cal_MDD(self.df_backtest.loc[:, 'total_value'])
        ...
        self.decomposition_summary = pd.Series(self.df_backtest.loc[:, ['gamma_pnl', 'vega_pnl', 'theta_pnl', 'higher_order_pnl',
                                        'unhedged_pnl', 'trading_cost', 'total_value']].sum().values, index=['total_gamma_pnl',
    ...
    def hedge_pnl_plot(self):
        df_plot = self.df_backtest.loc[:, ['option_value', 'stock_value', 'cash_account', 'total_value',
        ...
        ax1.plot(self.trade_dates, df_plot.loc[:, 'total_value'], linewidth=1, color='red', label='total_value')
        ...
        ax1.set_title('收益情况图 对冲方法：{0:s},总损益：{1:,.0f},期权损益：{2:,.0f}\n对冲损益：{3:,.0f},交易成本：{4:,.0f},现金账户最小值：{5:,.0f},最大回撤：{6:,.0f}'.
                      format(strategy_name, *self.hedge_pnl_summary.values))
        # plt.show()
        fig.savefig('../03_img/股票对冲回测.jpg')

    def pnl_decomposition_plot(self):
        df_plot = self.df_backtest.loc[:, ['delta_exposure_pnl', 'gamma_pnl', 'theta_pnl', 'vega_pnl',
                                           'higher_order_pnl', 'trading_cost']].cumsum()
        df_plot.loc[:,'total_value'] = self.df_backtest.loc[:,'total_value']
        ...
        ax1.set_xlabel('Date')
        ax1.set_ylabel('Value')
        ax1.plot(self.trade_dates, df_plot.loc[:, 'total_value'], linewidth=0.5, label='total_value')
        ax1.plot(self.trade_dates, df_plot.loc[:, 'delta_exposure_pnl'], linewidth=0.5, label='delta_value')

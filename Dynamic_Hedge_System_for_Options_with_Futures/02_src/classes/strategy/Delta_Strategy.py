...
class WW_Hedge(StrategyBase):
    ...
    def reset(self):
        self.WW_Hedge_df = None

    def calculate_target_delta_interval(self):
        portfolio_greek_df = self.option_greek_df
        public_df = self.public_df
        self.WW_Hedge_df = pd.DataFrame(0, index=public_df.index, columns=['H0','delta', 'delta_upper_bound', 'delta_lower_bound', 'target_delta'])
        r_list = []
...
class Zakamouline(StrategyBase):
    ...
    def reset(self):
        self.Zaka_Hedge_df = None

    def calculate_target_delta_interval(self):
        portfolio_greek_df = self.option_greek_df
        public_df = self.public_df
        self.Zaka_Hedge_df = pd.DataFrame(0, index=public_df.index, columns=['H0', 'H1', 'adj_delta', 'delta_upper_bound', 'delta_lower_bound', 'target_delta'])
        r_list = []
        for i in range(len(self.single_option_info)):
            option_obj = self.single_option_info[i]
